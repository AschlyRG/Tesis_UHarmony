...
# Agrega las funciones necesarias como locate_images, merge_recs, Rectangle, Note y otras funciones de procesamiento de imagen
...
def generate_midi(note_groups):
    ...
    if platform=='android':
        midi_file = file_path=join(app_storage_path(),"output.mid")
    else:
...
class UharmonyApp(App):
    ...
    def cargar_imagen(self, selection):
        ...
        if selection:
            ...
            # Elimina el archivo output.mid anterior 
                os.remove("./output.mid")
            else:
                print("El archivo no existe") 
            if platform=='android':
                from android_codes import show_toast
                show_toast(text="Iniciando el procesamiento de imagenes")
            try:
                threading.Thread(target=partial(self.procesar_imagen,img_file)).start() 
            except Exception as ex:
                print("Hubo un error al cargar la imagen",ex)

    def procesar_imagen(self, img_file):
        img_file_path = img_file
        print(f"La imagen se esta procesando : {img_file_path}")
        ...
        cargado = True 
        self.complted_task(midi_file)
    def play_again(self):
        if platform=='android':
            from android_codes import play_media,show_toast
               
             # En Android se usa el Media Player para reproducir archivos mid 
            
            file_path=join(app_storage_path(),"output.mid")
            if os.path.exists(file_path):
                
                play_media(file_path)
                show_toast(text="Reproduciendo de nuevo")
            else:
                show_toast(text="El archivo no existe")
        else:
            if os.path.exists("./output.mid"):
                from pygame import mixer
                mixer.init()
                mixer.music.load(r'./output.mid')
                mixer.music.play()
    ...
    @mainthread
    def complted_task(self,midi_file):
        
        print("La tarea se ha completado")
        if platform=='android':
            
            from android_codes import play_media,show_toast
            show_toast(text="Tarea completada")    
        ...
        if platform=='android':
            from android_codes import play_media,show_toast
            show_toast(text="Si la musica no se reproduce, quizas se produjo un error!") 
